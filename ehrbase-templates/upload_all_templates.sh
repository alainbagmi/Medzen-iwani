...
# Batch Upload Script for OpenEHR Templates
# Uploads all OPT files from opt-templates/ directory to EHRbase
# Usage: ./ehrbase-templates/upload_all_templates.sh
...
LOG_FILE="ehrbase-templates/upload_log_$(date +%Y%m%d_%H%M%S).txt"
MAX_RETRIES=3
RETRY_DELAY=5
...
# Print header
echo -e "${BLUE}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   📤 OpenEHR Template Batch Upload Script                ║${NC}"
echo -e "${BLUE}║   EHRbase URL: ${EHRBASE_URL%%/rest*}${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════════════╝${NC}\n"

# Initialize log file
echo "OpenEHR Template Upload Log - $(date)" > "$LOG_FILE"
echo "========================================" >> "$LOG_FILE"
echo "" >> "$LOG_FILE"

# Check if opt-templates directory exists
if [ ! -d "$OPT_DIR" ]; then
    echo -e "${RED}❌ ERROR: Directory not found: $OPT_DIR${NC}"
    echo -e "${YELLOW}ℹ️  Please ensure OPT templates have been converted from ADL format${NC}"
    echo -e "${YELLOW}ℹ️  Expected directory: $OPT_DIR${NC}"
    exit 1
fi
...
if [ ! -e "${OPT_FILES[0]}" ]; then
    ...
    echo -e "${YELLOW}ℹ️  See OPENEHR_TEMPLATE_DEPLOYMENT_GUIDE.md for conversion instructions${NC}"
    exit 1
fi
