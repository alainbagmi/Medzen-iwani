...
export const handler = async (event) => {
  ...
  try {
    ...
    let body;
    try {
      // Handle both API Gateway (event.body exists) and direct invoke (event is the body)
      if (event.body) {
        body = typeof event.body === 'string' ? JSON.parse(event.body) : event.body;
      } else {
        // Direct Lambda invoke - event IS the body
        body = event;
      }
    } catch (e) {
    ...
    // Store user message with correct database schema
    ...
    // Store AI response with correct database schema
