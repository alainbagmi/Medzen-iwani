-- =====================================================
-- Standardize Medical Provider Types
-- =====================================================
-- Creates standardized provider type enum and migrates existing data
-- to use consistent, validated professional role values
--
-- Created: 2025-11-06
-- Purpose: Enforce standardized provider type values across the system
-- =====================================================
...
-- Create enum type for professional roles
DO $$ BEGIN
...
-- =====================================================
-- This table provides additional metadata for each provider type
...
-- =====================================================
-- Update existing providers with non-standard values

-- Backup old values in a temporary column
...
-- Migrate common variations to standard values
UPDATE medical_provider_profiles
...
-- Grant SELECT on new table and views
...
-- SELECT * FROM provider_types ORDER BY display_order;

-- Check migrated values
-- SELECT
--   professional_role_legacy,
--   professional_role,
--   COUNT(*) as count
-- FROM medical_provider_profiles
-- GROUP BY professional_role_legacy, professional_role
-- ORDER BY count DESC;

-- Check for any unmigrated values
-- SELECT id, user_id, professional_role, professional_role_legacy
-- FROM medical_provider_profiles
-- WHERE professional_role IS NULL OR professional_role NOT IN (
--   'Dentist', 'Doctor of Osteopathic Medicine', 'Emergency Medical Technician',
--   'Licensed Clinical Social Worker', 'Medical Doctor', 'Medical Technologist',
--   'Nurse Practitioner', 'Occupational Therapist', 'Optometrist', 'Pharmacist',
--   'Physical Therapist', 'Physician Assistant', 'Psychologist', 'Registered Nurse',
--   'Respiratory Therapist'
-- );
...
-- =====================================================
-- Summary:
-- - Created provider_type enum with 15 standardized types
-- - Created provider_types lookup table with metadata
-- - Migrated existing data from common variations
-- - Added check constraint to enforce valid values
-- - Created view for easy querying with type details
-- - Preserved legacy values for reference
-- =====================================================
