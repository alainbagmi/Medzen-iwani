-- =====================================================
-- Migration: Fix nearby_places column references and add blood donor search
-- =====================================================

-- =====================================================
-- STEP 1: Fix the combined nearby_places function (correct column names)
-- =====================================================
...
CREATE OR REPLACE FUNCTION nearby_places(
  ...
  p_types TEXT[] DEFAULT NULL,        -- Filter: 'facility', 'provider', 'patient', facility_type, specialty
  p_limit INTEGER DEFAULT 50
)
RETURNS TABLE (
  place_kind TEXT,           -- 'facility', 'provider', or 'patient'
  id UUID,
  name TEXT,
  subtype TEXT,              -- facility_type, provider specialty, or 'blood_donor'
  image_url TEXT,
  phone_number TEXT,
  address TEXT,
  city TEXT,
...
BEGIN
  ...
  WITH all_places AS (
    ...
    -- MEDICAL PROVIDERS (with corrected column names)
    SELECT
...
-- =====================================================
-- STEP 2: Fix the get_nearby_providers function (correct column names)
-- =====================================================
...
-- =====================================================
-- STEP 3: Create function to find nearby blood donors (for providers)
-- =====================================================
...
-- =====================================================
-- STEP 4: Create function to find nearby patients (generic)
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    u.phone_number,
    u.country,
    COALESCE(u.blood_donation, false) as is_blood_donor,
    pp.blood_type,
    u.latitude,
    u.longitude,
    ROUND((ST_Distance(u.geog, v_user_point) / 1000.0)::numeric, 2)::double precision as distance_km,
  ...
  WHERE u.is_active = true
    AND u.geog IS NOT NULL
    AND ST_DWithin(u.geog, v_user_point, max_distance_km * 1000)
    AND (NOT blood_donors_only OR u.blood_donation = true)
  ORDER BY distance_km ASC
  LIMIT result_limit;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;
...
-- =====================================================
-- Done! All nearby search functions are now working
-- =====================================================
