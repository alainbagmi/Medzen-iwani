-- Fix UUID type mismatch in queue_role_profile_sync function
-- Error: column "record_id" is of type uuid but expression is of type text
-- Fix: Remove ::TEXT cast on NEW.id - insert UUID directly into UUID column
--
-- Reference: CLAUDE.md PostgreSQL Type Casting Rules
-- NEVER cast UUID to TEXT when inserting into UUID columns
...
BEGIN
  ...
  INSERT INTO ehrbase_sync_queue (
    ...
    'user_profiles',
    NEW.id,  -- ✅ FIXED: Removed ::TEXT cast - record_id is UUID, NEW.id is UUID
    v_template_id,
    'role_profile_create',
    'pending',
    NEW.role,
...
COMMENT ON FUNCTION queue_role_profile_sync IS 'Queues role-specific profile composition for EHRbase sync. Fixed UUID type mismatch - removed ::TEXT cast on record_id.';
