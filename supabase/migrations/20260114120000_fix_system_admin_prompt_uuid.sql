-- Migration: Fix System Admin System Prompt Update UUID
-- Date: 2026-01-14
-- Purpose: Correct the UUID in System Admin prompt update (original migration 20260114105000 used wrong UUID)
--
-- Root cause: Migration 20251218160000 deleted all old assistants and created new ones with different UUIDs
-- Original migration used outdated UUID 'b2c3d4e5-6789-01bc-def1-333333333333' which no longer exists
...
BEGIN
  ...
  IF v_contains_functions AND v_prompt_length > 2000 THEN
    RAISE NOTICE 'Migration successful: System Admin System Prompt updated with correct UUID (prompt length: %)', v_prompt_length;
    RAISE NOTICE '✓ System Admin assistant now has database function instructions';
  ELSE
